...
interface IStorage {
  ...
  function findBydate(array $params = []);
  function findAlldate(array $params = []);
}
...
class Storage implements IStorage {
  ...
  public function findBydate(array $params = [])
  {
    $found_items = $this->findAlldate($params);
    $first_index = array_keys($found_items)[0] ?? NULL;
    return $found_items[$first_index] ?? NULL;
  }

  public function findAlldate(array $params = [])
  {
    return array_filter($this->contents, function ($item) use ($params) {
      foreach ($params as $key => $value) {
        if (strtotime(((array)$item)[$key]) !== strtotime($value)) {
          return FALSE;
        }
      }
      return TRUE;
